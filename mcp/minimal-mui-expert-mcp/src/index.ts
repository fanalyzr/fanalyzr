...
The template is designed for rapid development of admin dashboards, client portals, and web applications with a professional, modern design system.`
      ...
      case "get_component_info":
        ...
        } else if (MINIMAL_MUI_KNOWLEDGE.components[componentType as keyof typeof MINIMAL_MUI_KNOWLEDGE.components]) {
          const components = MINIMAL_MUI_KNOWLEDGE.components[componentType as keyof typeof MINIMAL_MUI_KNOWLEDGE.components];
          componentInfo = `## ${componentType.charAt(0).toUpperCase() + componentType.slice(1)} Components:\n${components.map(c => `• ${c}`).join('\n')}`;
        } else {
          componentInfo = "Unknown component type. Available types: layout, form, common, dataDisplay, all";
        }
...
The theme system is fully integrated with MUI's theming capabilities, allowing for comprehensive customization while maintaining design consistency.`
      ...
      case "troubleshoot_issue":
        ...
        const commonIssue = MINIMAL_MUI_KNOWLEDGE.troubleshooting.common_issues.find(
          item => issueDescription?.toLowerCase().includes(item.issue.toLowerCase().split(' ')[0])
        );
